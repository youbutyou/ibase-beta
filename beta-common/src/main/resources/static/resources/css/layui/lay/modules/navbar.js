...
layui.define(['element', 'common'], function (exports) {
    ...
    function getHtml(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (data[i].children !== undefined && data[i].children !== null && data[i].children.length > 0) {
                ulHtml += '<a href="javascript:;"' + getDataSetHtml(data[i].url,data[i].sn,data[i].type) + ">" + data[i].title;
                ...
                for (var j = 0; j < data[i].children.length; j++) {
                    ...
                    if (data[i].children[j].children !== undefined && data[i].children[j].children !== null && data[i].children[j].children.length > 0) {
                        ulHtml += '<dd>';
                        ulHtml += '<a href="javascript:;"' + getDataSetHtml(data[i].children[j].url,data[i].children[j].sn, data[i].children[j].type) + ">" + data[i].children[j].title;
                        ulHtml += '<span class="layui-nav-more"></span>';
                        ulHtml += '</a>';
                        //三级菜单
                        ulHtml += '<dl class="layui-nav-child">';
                        ...
                        for (var k = 0; k < grandsonNodes.length; k++) {
                            ulHtml += '<dd>';
                            ulHtml += '<a href="javascript:;"'+ getDataSetHtml(grandsonNodes[k].url,grandsonNodes[k].sn,grandsonNodes[k].type) +'>' + grandsonNodes[k].title + '</a>';
                            ulHtml += '</dd>';
                        }
                        ulHtml += '</dl>';
                        ulHtml += '</dd>';
     ...
     * @returns {string}
     */
    function getDataSetHtml(url, sn, type) {
        var dataUrl = (url !== undefined && url !== '') ? ' data-url="' + url + '"' : '';
        var dataSn = (sn !== undefined && sn !== '') ? ' data-sn="' + sn + '"' : '';
        var dataType = (type !== undefined && type !== '') ? ' data-type="' + type + '"' : '';
        var html = dataUrl + dataSn + dataType;
        return html;
    }
